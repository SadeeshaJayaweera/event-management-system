...
export function CreateEvent({ onSave, onCancel, initialData }: CreateEventProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    image: initialData?.imageUrl || null as string | null
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      onSave({ ...formData, id: initialData?.id, price: Number(formData.price) || 0 });
      setLoading(false);
    }, 1000);
  };
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="0.00"
                    disabled={isEditing}
                    className={`w-full pl-8 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isEditing ? 'bg-gray-100 border-gray-200 text-gray-400 cursor-not-allowed' : 'border-gray-200'
                      }`}
                    value={formData.price}
                    onChange={(e) => !isEditing && setFormData({ ...formData, price: e.target.value })}
                  />
                </div>
                {isEditing ? (
